...
public class UserProfileController {
    ...
    public ResponseEntity<?> deleteFriendRequest(@PathVariable Long userId, @PathVariable Long profileId){
        ...
        if(friendsModel2.isPresent()){
            ...
            friendsModel.setDeletedAt(GlobalUtilities.formatDateAndTime(LocalDateTime.now()));
            friendsRequest.save(friendsModel);
            UserUpdateResponse userUpdateResponse = new UserUpdateResponse(GlobalUtilities.API_SUCCESS_STATUS, "Deleted successfully");
            return ResponseEntity.status(HttpStatus.ACCEPTED).body(userUpdateResponse);
        }
        UserUpdateResponse userUpdateResponse = new UserUpdateResponse(GlobalUtilities.API_FAILURE_STATUS, "Something went wrong");
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(userUpdateResponse);
